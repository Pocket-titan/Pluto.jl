...
import {
  create_counter_statistics,
  send_statistics_if_enabled,
  store_statistics_sample,
  finalize_statistics,
  init_feedback,
  ...
  return html`
    <aside id="export">
      <div id="container">
        ...
        <a
          href="./notebookfile?id=${notebook.notebook_id}"
          target="_blank"
          class="export_card"
        >
        ...
        >
          ...
          <section>
            An <b>.html</b> file for your web page, or to share online.
          </section>
        </a>
        <a
          href="#"
          ...
          onClick=${() => {
            if (window.chrome == null) {
              alert(
                "PDF generation works best on Google Chome.\n\n(We're working on it!)"
              );
            }
            window.print();
          }}
...
export class Editor extends Component {
  constructor() {
    ...
    this.actions = {
      ...
      serialize_selected: (cell) => {
        const selected = cell
          ? this.selected_friends(cell.id)
          : this.state.notebook.cells.filter((c) => c.selected);
        if (selected.length) {
      ...
      add_local_cell: (cell, new_index) => {
        return set_notebook_state((prevstate) => {
          ...
          return {
            cells: [
              ...before.slice(0, new_index),
              cell,
              ...before.slice(new_index),
            ],
          };
        });
      },
      update_local_cell_output: (
        cell,
        { output, queued, running, runtime, errored }
      ) => {
      ...
      move_local_cells: (cells, new_index) => {
        return set_notebook_state((prevstate) => {
          ...
          const old_first_index = prevstate.cells.findIndex((c) =>
            cells.includes(c)
          );

          const before = prevstate.cells.filter(
            (c, i) => i < new_index && !cells.includes(c)
          );
          const after = prevstate.cells.filter(
            (c, i) => i >= new_index && !cells.includes(c)
          );
    ...
    const on_update = (update, by_me) => {
      ...
      if (update.notebook_id == null) {
        ...
        if (this.state.notebook.notebook_id === update.notebook_id) {
          ...
          const cell = this.state.notebook.cells.find(
            (c) => c.cell_id == update.cell_id
          );
          switch (update.type) {
            case "cell_output":
              if (cell != null) {
            ...
            case "cell_input":
              if (cell != null) {
                this.actions.update_local_cell_input(
                  cell,
                  by_me,
                  message.code,
                  message.folded
            ...
            case "cell_deleted":
              ...
              break;
            case "cells_moved":
              const cells = message.cells.map((cell_id) =>
                this.state.notebook.cells.find((c) => c.cell_id == cell_id)
              );
    ...
    const on_establish_connection = (client) => {
      ...
      const run_all = this.client.session_options.evaluation
    ...
    this.requests = {
      ...
      split_remote_cell: async (cell_id, boundaries, submit = false) => {
        const index = this.state.notebook.cells.findIndex(
          (c) => c.cell_id == cell_id
        );
        ...
        const parts = boundaries
          .map((b, i) => slice_utf8(old_code, padded_boundaries[i], b).trim())
          .filter((x) => x !== "");
        ...
        for (const [i, part] of parts.entries()) {
          if (i === 0) {
            ...
            const update = await this.requests.add_remote_cell_at(
              index + i,
              true
            );
        ...
        await Promise.all(
          parts.map(async (part, i) => {
            ...
            await this.actions.update_local_cell_input(
              { cell_id: id },
              false,
              part,
              false
            ...
            const new_state = this.state.notebook.cells.find(
              (c) => c.cell_id === id
            );
        ...
        if (submit) {
          const cells = new_ids.map((id) =>
            this.state.notebook.cells.find((c) => c.cell_id == id)
          );
      ...
      confirm_delete_multiple: (verb, cells) => {
        if (cells.length <= 1 || confirm(`${verb} ${cells.length} cells?`)) {
          if (cells.some((f) => f.running || f.queued)) {
            if (
              confirm(
                "This cell is still running - would you like to interrupt the notebook?"
              )
            ) {
      ...
      set_and_run_all_changed_remote_cells: () => {
        const changed = this.state.notebook.cells.filter((cell) =>
          code_differs(cell)
        );
    ...
    document.addEventListener("keydown", (e) => {
      if (e.key === "q" && has_ctrl_or_cmd_pressed(e)) {
        ...
        (e.key === "?" && has_ctrl_or_cmd_pressed(e)) ||
        e.key === "F1"
      ) {
    ...
    window.addEventListener("beforeunload", (event) => {
      const first_unsaved = this.state.notebook.cells.find((cell) =>
        code_differs(cell)
      );
      if (first_unsaved != null) {
        window.dispatchEvent(
          new CustomEvent("cell_focus", {
            detail: { cell_id: first_unsaved.cell_id },
          })
        );
    ...
    setTimeout(() => {
      ...
      finalize_statistics(
        this.state,
        this.client,
        this.counter_statistics
      ).then(store_statistics_sample);
  ...
  componentDidUpdate() {
    ...
    const any_code_differs = this.state.notebook.cells.some((cell) =>
      code_differs(cell)
    );
    ...
    const all_completed_now = !this.state.notebook.cells.some(
    ...
    return html`
      ...
      <header className=${export_menu_open ? "show_export" : ""}>
        ...
        <nav id="at_the_top">
          <a href="./">
            <h1>
              <img id="logo-big" src="img/logo.svg" alt="Pluto.jl" /><img
                id="logo-small"
                src="img/favicon_unsaturated.svg"
              />
            </h1>
          </a>
          <${FilePicker}
            client=${this.client}
            value=${this.state.notebook.in_temp_dir
              ? ""
              : this.state.notebook.path}
            on_submit=${this.submit_file_change}
            suggest_new_file=${{
              base:
                this.client.session_options == null
                  ? ""
                  : this.client.session_options.server.notebook_path_suggestion,
      ...
      <main>
        <preamble>
          <button
            onClick=${() =>
              this.requests.set_and_run_all_changed_remote_cells()}
            class="runallchanged"
        ...
        <${Notebook}
          ...
          on_update_doc_query=${(query) =>
            this.setState({ desired_doc_query: query })}
          ...
          on_focus_neighbor=${(cell_id, delta) => {
            const i = this.state.notebook.cells.findIndex(
              (c) => c.cell_id === cell_id
            );
            const new_i = i + delta;
        ...
        <${SelectionArea}
          ...
          on_selection=${(selected_cell_ids) => {
            let current_selected_cells = this.state.notebook.cells
              .filter((x) => x.selected)
              .map((x) => x.cell_id);
      ...
      <${UndoDelete}
        ...
        on_click=${() => {
          this.requests
            .add_remote_cell_at(this.state.recently_deleted.index, true)
            .then((update) => {
              this.on_update(update, true);
              this.actions
                .update_local_cell_input(
                  { cell_id: update.cell_id },
                  false,
                  this.state.recently_deleted.body,
                  false
                )
                .then(() => {
      ...
      <footer>
        <div id="info">
          <form id="feedback" action="#" method="post">
            ...
            <label for="opinion"
              >🙋 How can we make
              <a href="https://github.com/fonsp/Pluto.jl">Pluto.jl</a>
              better?</label
            >
            <input
              type="text"
