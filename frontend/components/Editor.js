...
export class Editor extends Component {
  constructor() {
    ...
    this.actions = {
      ...
      add_deserialized_cells: async (data, index) => {
        ...
        for (const cell of new_cells) {
          const cm = document.querySelector(`[id="${cell.cell_id}"] .CodeMirror`)
            .CodeMirror;
      ...
      move_remote_cells: (cell_ids, new_index) => {
        update_notebook((notebook) => {
          let before = notebook.cell_order
            .slice(0, new_index)
            .filter((x) => !cell_ids.includes(x));
          let after = notebook.cell_order
            .slice(new_index, Infinity)
            .filter((x) => !cell_ids.includes(x));
      ...
      confirm_delete_multiple: async (verb, cell_ids) => {
        if (cell_ids.length <= 1 || confirm(`${verb} ${cell_ids.length} cells?`)) {
          if (
            cell_ids.some(
              (cell_id) =>
                this.state.notebook.cell_results[cell_id].running ||
      ...
      set_and_run_all_changed_remote_cells: () => {
        const changed = this.state.notebook.cell_order.filter(
          (cell_id) =>
            ...
            this.state.notebook.cell_inputs[cell_id].code !==
              this.state.cell_inputs_local[cell_id]?.code
        );
          ...
          await update_notebook((notebook) => {
            for (let cell_id of cell_ids) {
              if (this.state.cell_inputs_local[cell_id]) {
                notebook.cell_inputs[cell_id].code = this.state.cell_inputs_local[
                  cell_id
                ].code;
              }
            }
            ...
            if (message.patches.length !== 0) {
              this.setState(
                immer((state) => {
                  ...
                  let cells_stuck_in_limbo = new_notebook.cell_order.filter(
                    (cell_id) => new_notebook.cell_inputs[cell_id] == null
                  );
                  if (cells_stuck_in_limbo.length !== 0) {
                    console.warn(`cells_stuck_in_limbo:`, cells_stuck_in_limbo);
                    new_notebook.cell_order = new_notebook.cell_order.filter(
                      (cell_id) => new_notebook.cell_inputs[cell_id] != null
                    );
                  }
    ...
    const on_establish_connection = async (client) => {
      ...
      setTimeout(() => {
        init_feedback();
        finalize_statistics(this.state, this.client, this.counter_statistics).then(
          store_statistics_sample
        );
    ...
    this.notebook_is_idle = () =>
      !Object.values(this.state.notebook.cell_results).some(
    ...
    let update_notebook = async (mutate_fn) => {
      ...
      let [new_notebook, changes, inverseChanges] = produceWithPatches(
        this.state.notebook,
        (notebook) => {
          mutate_fn(notebook);
        }
      ...
      if (DEBUG_DIFFING) {
        try {
          let previous_function_name = new Error().stack
            .split("\n")[2]
            .trim()
            .split(" ")[1];
          console.log(
            `Changes to send to server from "${previous_function_name}":`,
            changes
          );
        } catch (error) {}
      }
          ...
          this.client
            ...
            .then((response) => {
              if (response.message.response.update_went_well === "👎") {
                ...
                throw new Error(
                  `Pluto update_notebook error: ${response.message.response.why_not})`
                );
              }
            }),
          new Promise((resolve) => {
    ...
    this.serialize_selected = (cell_id = null) => {
      const cells_to_serialize =
        cell_id == null || this.state.selected_cells.includes(cell_id)
          ? this.state.selected_cells
          : [cell_id];
      if (cells_to_serialize.length) {
        return serialize_cells(
          cells_to_serialize.map((id) => this.state.notebook.cell_inputs[id])
        );
      }
    };
    ...
    window.addEventListener("beforeunload", (event) => {
      const unsaved_cells = this.state.notebook.cell_order.filter(
        (id) =>
          this.state.cell_inputs_local[id] &&
          this.state.notebook.cell_inputs[id].code !==
            this.state.cell_inputs_local[id].code
      );
      const first_unsaved = unsaved_cells[0];
      if (first_unsaved != null) {
        window.dispatchEvent(
          new CustomEvent("cell_focus", { detail: { cell_id: first_unsaved } })
        );
  ...
  componentDidUpdate(old_props, old_state) {
    ...
    document.body.classList.toggle(
      "loading",
      this.state.initializing || this.state.moving_file
    );
                    ...
                    <header className=${export_menu_open ? "show_export" : ""}>
                        ...
                        <nav id="at_the_top">
                            ...
                            <${FilePicker}
                                client=${this.client}
                                value=${
                                  this.state.notebook.in_temp_dir
                                    ? ""
                                ...
                                suggest_new_file=${{
                                  base:
                                    this.client.session_options == null
                                      ? ""
                                      : this.client.session_options.server
                                          .notebook_path_suggestion,
                                  name: this.state.notebook.shortpath,
                                }}
                                placeholder="Save notebook..."
                                button_label=${
                                  this.state.notebook.in_temp_dir ? "Choose" : "Move"
                                }
                            />
                        ...
                        <${SelectionArea}
                            ...
                            on_selection=${(selected_cell_ids) => {
                              ...
                              if (
                                ...
                                _.difference(selected_cell_ids, this.state.selected_cells)
                                  .length !== 0
                              ) {
                                this.setState({
                    ...
                    <${UndoDelete}
                        ...
                        on_click=${() => {
                          this.update_notebook((notebook) => {
                            ...
                            this.actions.set_and_run_multiple(
                              this.state.recently_deleted.map(({ cell }) => cell.cell_id)
                            );
                          });
                        }}
                    />
