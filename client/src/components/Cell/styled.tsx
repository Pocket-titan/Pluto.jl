...
const Container = styled.div<{ selected: boolean; selectedByFriends?: string }>`
  position: relative;
  margin-top: 2.04em;
  box-shadow: 3px 3px 9px hsla(0, 0%, 0%, 0.25);
  /* Line height in monaco with fontSize: 17 (current default) TODO: maybe do editor.getOption() instead of hardcoding */
  min-height: 23px;

  ${({ selected, selectedByFriends, theme }) =>
    (selected || selectedByFriends) &&
    css`
      &:after {
        ...
        background-color: ${selected
          ? theme.isDark
            ? "hsla(207, 84%, 70%, 0.2)"
            : "rgba(20, 40, 140, 0.2)"
          : tinycolor(selectedByFriends).setAlpha(0.1).toRgbString()};
      }
    `}
`;
