...
function createStyleElementForColorsCSS(): HTMLStyleElement {
  ...
  const my_secret_className = "__my_secret_monaco_className";
  style.className = my_secret_className;

  // Don't forget to remove our previously injected styles!
  Array.from(
    document.getElementsByClassName(my_secret_className)
  ).forEach((oldStyle) => oldStyle.remove());
