...
"A combination of _client ID_ and a _request ID_. The front-end generates a unqique ID for every request that it sends. The back-end (the stuff you are currently reading) can respond to a specific request. In that case, the response does not go through the normal body handlers in the front-end, but it flies directly to the place where the body was sent. (It resolves the promise returned by `send(...)`.)"
struct Initiator
    client_id::Symbol
    request_id::Symbol
end
...
UpdateMessage(type::Symbol, body::Any) = UpdateMessage(type, body, nothing, nothing, missing)
UpdateMessage(type::Symbol, body::Any, notebook::Notebook) = UpdateMessage(type, body, notebook, nothing, missing)
